...
function setupEventListeners() {
    ...
    async function savePartChange(part) {
    ...
    const debouncedSave = debounce(savePartChange, 750);
    ...
    clearDataLink.addEventListener('click', (e) => {
        ...
        openConfirmModal(
            ...
            async () => {
                showLoadingOverlay();
                try {
                    await replaceAllOrdersOnBackend([]);
                    state.orders = [];
                    state.klanten = DEFAULT_KLANTEN;
                    state.machines = DEFAULT_MACHINES;
                    saveState();
                    renderAll();
            ...
            try {
                ...
                openConfirmModal(
                    ...
                    async () => {
                        showLoadingOverlay();
                        try {
                            await replaceAllOrdersOnBackend(importedData.orders || []);
                            state.orders = importedData.orders || [];
                            state.klanten = importedData.klanten || DEFAULT_KLANTEN;
                            state.machines = importedData.machines || DEFAULT_MACHINES;
    ...
    window.addEventListener('click', (e) => {
        if (!document.getElementById('actions-dropdown-container').contains(e.target)) {
            actionsDropdownMenu.classList.add('hidden');
        }
    });
    ...
    machineListUl.addEventListener('click', (e) => {
        if (e.target.classList.contains('delete-machine-btn')) {
            ...
            const isMachineInUse = state.orders.some(o => o.parts.some(p => p.machine === machineToDelete));
            ...
            const title = 'Machine Verwijderen';
            let text = `Weet je zeker dat je machine "${machineToDelete}" wilt verwijderen?`;

            if (isMachineInUse) {
                text = `Let op: Machine "${machineToDelete}" is momenteel in gebruik. Als je doorgaat, worden de betreffende onderdelen losgekoppeld. Weet je het zeker?`;
            }

            openConfirmModal(title, text, deleteLogic);
        }
    });
